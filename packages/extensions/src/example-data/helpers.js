 ...
 * ```
 */
export function loadVocabulary(language) {
 ...
 * ```
 */
export function findWord(word, language) {
 ...
 * ```
 */
export function getTranslation(word, sourceLanguage, targetLanguage) {
 ...
 * ```
 */
export function getTranscription(word, language, system) {
 ...
 * ```
 */
export function getAllTranscriptions(word, language) {
    const entry = findWord(word, language);
    if (!entry)
        return undefined;
 ...
 * ```
 */
export function getWordsByProperty(language, property, value) {
    const dataset = loadVocabulary(language);
    if (!dataset)
        return [];
...
export function getRandomWord(language) {
    const dataset = loadVocabulary(language);
    if (!dataset || dataset.vocabulary.length === 0)
        return undefined;
 ...
 * ```
 */
export function countVocabulary(language) {
    const dataset = loadVocabulary(language);
