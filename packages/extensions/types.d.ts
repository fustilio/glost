 ...
 */
import type { GLOSTWord, GLOSTSentence, GLOSTParagraph, GLOSTRoot, GLOSTExtras } from "glost-core";
/**
...
export interface GLOSTExtension {
     ...
     * ```
     */
    transform?: (tree: GLOSTRoot, context?: ExtensionContext) => GLOSTRoot | Promise<GLOSTRoot>;
    /**
    ...
    visit?: {
         ...
         */
        word?: (node: GLOSTWord, context?: ExtensionContext) => GLOSTWord | void | Promise<GLOSTWord | void>;
        /**
         ...
         * @returns Modified node or void (or Promise for async operations)
         */
        sentence?: (node: GLOSTSentence, context?: ExtensionContext) => GLOSTSentence | void | Promise<GLOSTSentence | void>;
        /**
         ...
         * @returns Modified node or void (or Promise for async operations)
         */
        paragraph?: (node: GLOSTParagraph, context?: ExtensionContext) => GLOSTParagraph | void | Promise<GLOSTParagraph | void>;
    };
    /**
     ...
     * ```
     */
    enhanceMetadata?: (node: GLOSTWord, context?: ExtensionContext) => Partial<GLOSTExtras> | void | Promise<Partial<GLOSTExtras> | void>;
    /**
    ...
    /**
     * Streaming support mode for this extension
     *
     * Controls how this extension participates in chunked streaming:
     * - `'none'` (default): runs once on the full document before
     *   streaming begins. Use for extensions with `transform` that need
     *   the whole document in scope.
     * - `'chunk'`: receives each batch of sentences wrapped in a
     *   temporary GLOSTRoot. The extension's `visit` and
     *   `enhanceMetadata` hooks run per-chunk. `transform` is ignored
     *   in this mode.
     * - `'full'`: ignored in streaming mode (kept for backward
     *   compatibility with non-streaming pipelines).
     *
     * Extensions without this field default to `'none'`.
     *
     * @since 0.7.0
     *
     * @example
     * ```typescript
     * // Runs per-chunk during streaming
     ...
     * ```
     */
    streamingSupport?: "none" | "chunk" | "full";
}
/**
