 ...
 * @internal
 */
function hasNodeType(document, nodeType) {
    let found = false;
    visit(document, (node) => {
 ...
 * @internal
 */
function validateNodeRequirements(extension, document, options) {
    const errors = [];
    ...
    for (const nodeType of extension.requires.nodes) {
        if (!hasNodeType(document, nodeType)) {
            ...
            const error = new MissingNodeTypeError(extension.id, nodeType, provider?.id);
            if (options.lenient) {
 ...
 * @internal
 */
function findProviderForNode(nodeType) {
 ...
 * @since 0.0.1
 */
export function processGLOSTWithExtensions(document, extensions, options = {}) {
    ...
    try {
        ...
        for (const extension of orderedExtensions) {
            try {
                ...
                const validationErrors = validateNodeRequirements(extension, processedDocument, options);
                ...
                // Apply visitors
                if (extension.visit) {
                    processedDocument = applyVisitors(processedDocument, extension.visit, context);
                }
                // Apply metadata enhancement with deep merge
                if (extension.enhanceMetadata) {
                    processedDocument = enhanceMetadata(processedDocument, extension.enhanceMetadata, extension.id, fieldOwnership, options, context);
                }
 ...
 * @internal
 */
function applyVisitors(document, visitors, context) {
 ...
 * @internal
 */
function enhanceMetadata(document, enhancer, extensionId, fieldOwnership, options, context) {
        ...
        if (node.type === "WordNode") {
            ...
            if (enhancement) {
                ...
                node.extras = deepMerge(node.extras ?? {}, enhancement, {
                    ...
                    existingExtensionId: getOwnerForFields(node.extras ?? {}, enhancement, fieldOwnership),
 ...
 * @internal
 */
function getOwnerForFields(existing, incoming, fieldOwnership) {
 ...
 * @internal
 */
function trackFieldOwnership(enhancement, extensionId, fieldOwnership, prefix = "") {
    for (const [key, value] of Object.entries(enhancement)) {
        ...
        if (typeof value === "object" &&
            value !== null &&
            !Array.isArray(value)) {
            trackFieldOwnership(value, extensionId, fieldOwnership, fullKey);
        }
    }
}
/**
 ...
 * @since 0.0.1
 */
export function processGLOSTWithExtensionIds(document, extensionIds, options = {}) {
    const extensions = extensionIds
        .map((id) => extensionRegistry.get(id))
        .filter((ext) => ext !== undefined);
    if (extensions.length !== extensionIds.length) {
        const missing = extensionIds.filter((id) => !extensions.some((ext) => ext.id === id));
 ...
 * @since 0.0.1
 */
export async function processGLOSTWithExtensionsAsync(document, extensions, options = {}) {
    // Register extensions temporarily if not already registered
    const tempExtensions = [];
    for (const ext of extensions) {
        if (!extensionRegistry.has(ext.id)) {
    ...
    try {
        ...
        for (const extension of orderedExtensions) {
            try {
                ...
                if (extension.transform) {
                    processedDocument = await extension.transform(processedDocument, context);
                }
                // Apply visitors (supports async)
                if (extension.visit) {
                    processedDocument = await applyVisitorsAsync(processedDocument, extension.visit, context);
                }
                // Apply metadata enhancement (supports async)
                if (extension.enhanceMetadata) {
                    processedDocument = await enhanceMetadataAsync(processedDocument, extension.enhanceMetadata, extension.id, fieldOwnership, options, context);
                }
                appliedExtensions.push(extension.id);
                context.appliedExtensions.push(extension.id);
            }
 ...
 * ```
 */
export async function processGLOST(document, extensions, options) {
 ...
 * ```
 */
export async function processGLOSTWithMeta(document, extensions, options) {
 ...
 * @since 0.7.0
 */
export async function processGLOSTChunkAsync(sentences, extensions, options = {}) {
    if (sentences.length === 0) {
        return sentences;
    }
    // Wrap sentences in a temporary root so visitor machinery works.
    // The `script` field is required by GLOSTRoot; we use a neutral
    // placeholder since this root is never persisted.
    const tempParagraph = {
        type: "ParagraphNode",
        children: sentences,
    };
    const tempRoot = {
        ...
        lang: "und",
        script: "latin",
        children: [tempParagraph],
        metadata: {},
        extras: {},
    };
    // Run only visit and enhanceMetadata — never transform
    // Field ownership is local to this chunk (reset per chunk)
    const fieldOwnership = new Map();
    const context = {
        originalDocument: tempRoot,
    ...
    for (const extension of extensions) {
        try {
            if (extension.visit) {
                await applyVisitorsAsync(tempRoot, extension.visit, context);
            }
            if (extension.enhanceMetadata) {
                await enhanceMetadataAsync(tempRoot, extension.enhanceMetadata, extension.id, fieldOwnership, options, context);
            }
            context.appliedExtensions.push(extension.id);
        }
        catch (error) {
            ...
            // In lenient mode, skip the failing extension silently
        }
    }
    // Extract sentences back from the temporary paragraph
    const para = tempRoot.children[0];
    if (para &&
 ...
 * @internal
 */
async function applyVisitorsAsync(document, visitors, context) {
    let processed = document;
    ...
    if (visitors.word) {
        ...
        await Promise.all(wordNodes.map(async (node) => {
    ...
    if (visitors.sentence) {
        ...
        await Promise.all(sentenceNodes.map(async (node) => {
    ...
    if (visitors.paragraph) {
        ...
        await Promise.all(paragraphNodes.map(async (node) => {
 ...
 * @internal
 */
async function enhanceMetadataAsync(document, enhancer, extensionId, fieldOwnership, options, context) {
    const wordNodes = [];
    visit(document, "WordNode", (node) => {
        if (node.type === "WordNode") {
            wordNodes.push(node);
