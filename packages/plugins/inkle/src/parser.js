import { createGLOSTRootNode, createGLOSTParagraphNode, createGLOSTSentenceNode, createGLOSTWordNode, } from "glost";
export function parseInkJSON(json, options = {}) {
    ...
    if (options.sections) {
        ...
        if (Array.isArray(json.root)) {
            ...
            for (const key in json) {
                if (key === 'inkVersion' || key === 'listDefs')
                    continue;
...
function traverseContent(node, callback) {
        ...
        while (i < node.length) {
            ...
            // Choice: ev str ^Text /str /ev
                ...
                if (node[i + 1] === "str" &&
                    ...
                    node[i + 3] === "/str" &&
                    node[i + 4] === "/ev") {
                    const choiceText = node[i + 2].substring(1);
