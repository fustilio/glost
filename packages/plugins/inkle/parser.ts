import {
  createMTSTRootNode,
  createMTSTParagraphNode,
  createMTSTSentenceNode,
  createMTSTWordNode,
  createMTSTTextNode,
} from "@lalia/mtst";
import type {
...
export function parseInkJSON(
  ...
  const paragraphs: MTSTParagraph[] = [];

  const addLine = (text: string, type: "text" | "choice") => {
    if (!text.trim()) return;

    // Simple tokenization by space
    const words: MTSTWord[] = text.split(/\s+/).map((w) => {
      // Create a basic word node
      return createMTSTWordNode(
        w,
        {}, // No transcription
    ...
    const sentence = createMTSTSentenceNode(text, language, script, words);
    
    // Add extra metadata for choices
    const extras = type === 'choice' ? { type: 'choice' } : {};
    
    const paragraph = createMTSTParagraphNode([sentence], { ...extras });
  ...
  return createMTSTRootNode(language, script, paragraphs);
}
