 ...
 */
function getDifficultyHints(difficulty, includeEncouragement) {
    const hints = [];
    switch (difficulty) {
        case "advanced":
 ...
 * @since 0.0.2
 */
export function createLearnerHintsExtension(options = {}) {
    const includeGrammarTips = options.includeGrammarTips ?? true;
    const includeEncouragement = options.includeEncouragement ?? true;
    return {
        id: "learner-hints",
        name: "Learner Hints",
        description: "Generates contextual hints for language learners based on word metadata",
        ...
        enhanceMetadata: (node) => {
            ...
            const posData = node.extras?.partOfSpeech;
            const pos = posData?.tag;
            // Get difficulty from the extension's output
            const difficultyData = node.extras?.difficulty;
            const difficulty = difficultyData?.level;
            // Get gender (optional - may not be present)
            const genderData = node.extras?.gender;
            const gender = genderData?.display;
            // Strict validation for required fields
            if (!pos) {
                throw new ExtensionDependencyError("learner-hints", "part-of-speech", "extras.partOfSpeech.tag", "POS generator (from glost-pos) must run before LearnerHintsExtension. " +
                    "Add POS generator to your extension list.");
            }
            if (!difficulty) {
                throw new ExtensionDependencyError("learner-hints", "difficulty", "extras.difficulty.level", "Difficulty generator (from glost-difficulty) must run before LearnerHintsExtension. " +
...
//# sourceMappingURL=learner-hints.js.map