 ...
 */
function getReadingColor(score) {
    if (score <= 1.5)
        return "green";
    if (score <= 2.5)
        return "blue";
 ...
 * @since 0.0.2
 */
export function createReadingScoreExtension(options = {}) {
    const frequencyWeight = options.frequencyWeight ?? 0.4;
    const difficultyWeight = options.difficultyWeight ?? 0.6;
    return {
        id: "reading-score",
        name: "Reading Score",
        description: "Calculates composite reading score from frequency + difficulty",
        ...
        enhanceMetadata: (node) => {
            ...
            const frequencyData = node.extras?.frequency;
            ...
            if (!frequency) {
                throw new ExtensionDependencyError("reading-score", "frequency", "extras.frequency.level", "Frequency extension did not provide 'frequency.level' field. " +
            ...
            if (!difficulty) {
                throw new ExtensionDependencyError("reading-score", "difficulty", "extras.difficulty.level", "Difficulty extension did not provide 'difficulty.level' field. " +
            ...
            const score = frequencyScore * frequencyWeight + difficultyScore * difficultyWeight;
...
//# sourceMappingURL=reading-score.js.map