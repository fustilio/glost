 ...
 */
import { createGLOSTWordNode, createGLOSTSentenceNode, createGLOSTParagraphNode, createGLOSTRootNode, createGLOSTPunctuationNode, } from "glost-core";
/**
 ...
 * @since 0.0.1
 */
export function createMockGLOSTWord(text, options = {}) {
    const { lang = "en-US", script = "latin", transcription = {}, metadata = { partOfSpeech: "" }, extras = {}, difficulty, } = options;
    ...
    if (difficulty) {
        wordNode.difficulty = difficulty;
    }
    return wordNode;
}
/**
 ...
 * @since 0.0.1
 */
export function createMockGLOSTDocument(wordsOrOptions, options = {}) {
    ...
    const sentence = createGLOSTSentenceNode({ originalText: "", lang: lang, script: script, children: wordNodes });
    const paragraph = createGLOSTParagraphNode([sentence]);
    const root = createGLOSTRootNode({ lang: lang, script: script, children: [paragraph] });
    return root;
}
/**
 * Create a mock document from a specification object
 */
function createMockGLOSTDocumentFromSpec(spec) {
    ...
    const sentenceSpecs = sentences ??
        (words ? [{ words, lang, script }] : []);
    const sentenceNodes = sentenceSpecs.map((sentenceSpec) => {
        ...
        const wordNodes = sentenceWords.map((wordSpec) => {
        ...
        const punctuationNodes = sentencePunctuation.map((p) => createGLOSTPunctuationNode(p.text));
        // Create sentence with words
        const sentence = createGLOSTSentenceNode({
            originalText: sentenceSpec.originalText ?? "",
            lang: sentenceLang,
            script: sentenceScript,
            children: wordNodes,
        });
    ...
    if (sentenceNodes.length === 0) {
        const emptySentence = createGLOSTSentenceNode({ originalText: "", lang: lang, script: script, children: [] });
 ...
 * @since 0.0.1
 */
export function createMockExtension(id, options = {}) {
    const { name = `Test Extension ${id}`, description, enhanceMetadata, transform, visit, dependencies, } = options;
    return {
        id,
        name,
        description,
