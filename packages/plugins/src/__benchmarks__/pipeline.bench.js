...
import { createSimpleDocument, createGLOSTWordNode } from 'glost-core';
import { processGLOST, processGLOSTWithMeta, } from '../index.js';
...
function createSimpleExtension(id) {
    return {
        id,
...
function createProcessingExtension(id) {
    return {
        id,
...
// Async extension
function createAsyncExtension(id) {
    return {
        id,
...
describe('Extension Pipeline: Comparing Strategies', () => {
    ...
    describe('Compare: Extension Composition', () => {
        ...
        bench('5 extensions (sequential)', async () => {
            const extensions = Array.from({ length: 5 }, (_, i) => createSimpleExtension(`ext${i}`));
