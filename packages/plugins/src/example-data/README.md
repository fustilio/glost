...
**⚠️ Note:** This is an **opt-in export** - it's NOT included in the main `glost-plugins` bundle. Import it explicitly when needed for examples, demos, or testing.

## Installation & Usage

The example data is included in `glost-plugins` but as a separate export path:
...
```typescript
import { findWord, getTranscription } from 'glost-plugins/example-data';
```
