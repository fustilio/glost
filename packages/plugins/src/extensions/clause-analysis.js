...
const CLAUSE_DESCRIPTIONS = {
    ...
    adverbial: "Adverbial clause - modifies the verb by expressing time, cause, or condition",
};
/**
 * Complexity scores for different clause types
 */
const CLAUSE_COMPLEXITY = {
    main: 1,
    subordinate: 3,
    relative: 4,
    adverbial: 3,
 ...
 */
function checkForVerb(clause) {
    let hasVerb = false;
    visit(clause, "WordNode", (node) => {
 ...
 */
function calculateComplexity(clause, wordCount, hasVerb) {
 ...
 * @since 0.0.2
 */
export function createClauseAnalysisExtension(options = {}) {
    ...
    return {
        ...
        transform: (document, context) => {
            ...
            const segmenterRan = context?.appliedExtensions.includes("clause-segmenter");
            ...
            if (clauseCount === 0) {
                if (!segmenterRan) {
                    throw new ExtensionDependencyError("clause-analysis", "clause-segmenter", "ClauseNode nodes", "Clause segmenter (from glost-clause-segmenter) must run before ClauseAnalysisExtension. " +
                ...
                console.warn("[clause-analysis] No clauses found in document. " +
...
//# sourceMappingURL=clause-analysis.js.map