...
class ExtensionRegistry {
    ...
    register(extension) {
        if (this.extensions.has(extension.id)) {
            console.warn(`Extension with ID "${extension.id}" is already registered. Overwriting.`);
        }
     ...
     * ```
     */
    resolveDependencies(extensionIds) {
        const resolved = [];
        const visited = new Set();
        const visiting = new Set();
        const visit = (id) => {
            if (visiting.has(id)) {
                throw new Error(`Circular dependency detected in extensions: ${id}`);
            }
            if (visited.has(id)) {
                return;
            }
