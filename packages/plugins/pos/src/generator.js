 ...
 * ```
 */
export function createPOSGeneratorExtension(options) {
    ...
    return {
        ...
        visit: {
            word: async (node) => {
                // Skip if no provider
                if (!provider) {
                    console.warn("[POS Generator] No provider provided, skipping processing");
                    return;
                }
                // Skip if POS already exists
                if (skipExisting) {
                    const existingPOS = node.metadata?.partOfSpeech ||
                ...
                try {
                    const pos = await provider.getPOS(cleanWordText, options.targetLanguage);
                    if (pos) {
                    ...
                    console.debug(`[POS Generator] POS tagging failed for "${cleanWordText}":`, error);
                }
            },
        },
        options: options,
