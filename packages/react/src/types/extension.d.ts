...
export interface GloSTRenderExtension {
     ...
     */
    shouldApply?: (node: GLOSTWord, displayLevel: DisplayLevel, transcriptionSystem?: string) => boolean;
    /**
     * Get additional class names for the word container
     *
     * @returns CSS class names to add to the word container
     */
    getClassName?: (node: GLOSTWord, displayLevel: DisplayLevel, transcriptionSystem?: string, languageStrategy?: LanguageRenderingStrategy) => string;
    /**
     * Render content before the word text (inside the main container)
     */
    renderBefore?: (node: GLOSTWord, displayLevel: DisplayLevel, transcriptionSystem?: string, languageStrategy?: LanguageRenderingStrategy) => React.ReactNode;
    /**
     * Render content after the word text (inside the main container)
     */
    renderAfter?: (node: GLOSTWord, displayLevel: DisplayLevel, transcriptionSystem?: string, languageStrategy?: LanguageRenderingStrategy) => React.ReactNode;
    /**
     * Render additional content in the definition/metadata section
     */
    renderMetadata?: (node: GLOSTWord, displayLevel: DisplayLevel, transcriptionSystem?: string, languageStrategy?: LanguageRenderingStrategy) => React.ReactNode;
}
/**
 * Filter extensions that should apply for the current context
 */
export declare function getActiveExtensions(extensions: GloSTRenderExtension[], node: GLOSTWord, displayLevel: DisplayLevel, transcriptionSystem?: string): GloSTRenderExtension[];
/**
 * Combine class names from multiple extensions
 */
export declare function combineExtensionClassNames(extensions: GloSTRenderExtension[], node: GLOSTWord, displayLevel: DisplayLevel, transcriptionSystem?: string, languageStrategy?: LanguageRenderingStrategy): string;
