 ...
 */
export const JAPANESE_TRANSCRIPTION_SCHEME_NAMES = {
 ...
 * ```
 */
export function isJapaneseCharacter(char) {
 ...
 * ```
 */
export function containsJapaneseCharacters(text) {
 ...
 * ```
 */
export function isJapaneseText(text, allowSpaces = true) {
    if (!text)
        return false;
 ...
 * ```
 */
export function isValidJapaneseTranscriptionScheme(scheme) {
    return Object.values(JAPANESE_TRANSCRIPTION_SCHEMES).includes(scheme);
}
/**
 ...
 * ```
 */
export function getJapaneseTranscriptionSchemeName(scheme) {
