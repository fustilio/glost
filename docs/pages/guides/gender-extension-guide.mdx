...
```
📘 le    chat     [Masc] → Masculine (blue)
📘 le    chien    [Masc] → Masculine (blue)
📕 la    maison   [Fem]  → Feminine  (pink)
📕 la    voiture  [Fem]  → Feminine  (pink)
📘 le    livre    [Masc] → Masculine (blue)
📕 la    table    [Fem]  → Feminine  (pink)
```
...
// "chat" (masculine) + definite → "le"
// "maison" (feminine) + indefinite → "une"
```
...
// "chat" (masculine) + "petit" → "petit chat" (no -e)
// "maison" (feminine) + "petit" → "petite maison" (add -e)
```
...
// -tion → 95% feminine ("nation", "station", "action")
// -age → 85% masculine ("garage", "village", "message")
// -eur → 90% masculine ("acteur", "docteur", "professeur")
```
...
**Output with Gender**:
```
Der    [Art-Masc]  ♂ Masculine article
Mann   [N-Masc]    ♂ Masculine noun
und    [Conj]      - (and)
die    [Art-Fem]   ♀ Feminine article  
Frau   [N-Fem]     ♀ Feminine noun
lesen  [V]         - (read)
das    [Art-Neut]  ⚬ Neuter article
Buch   [N-Neut]    ⚬ Neuter noun
```
...
// "gato" + singular → "el gato"
// "gato" + plural → "los gatos"
// "mesa" + singular → "la mesa"
// "mesa" + plural → "las mesas"
```
...
// "la chat" → Error: "la" (feminine) doesn't match "chat" (masculine)
...
// Examples: nation ♀, station ♀, action ♀, solution ♀, option ♀
```
...
- ✅ Visual cues (colors, icons) reinforce gender
- ✅ Correct articles automatically shown (le vs la, der vs die vs das)
- ✅ Patterns discovered (-tion = feminine, -age = masculine)
- ✅ Agreement errors caught (la chat → le chat)
- ✅ Exercises generated automatically (fill article, match gender)
...
**Note**: Gender extension is only applicable to languages with grammatical gender (French, Spanish, German, Russian, etc.). For languages without grammatical gender (English, Thai, Japanese, Chinese), this extension returns `undefined` for all words.
