# GLOST Migration Guides

## Current Version: v0.5.0

This document contains the latest migration guide for upgrading to the current GLOST version.

## Latest Migration

### v0.4 → v0.5 (Current)

**See:** [Migration Guide v0.4 to v0.5](/docs/migration/v0.4-to-v0.5)

**Key Changes:**
- Unified-style processor API with fluent `.use()` chaining
- Package restructure: `glost` (core) → `@glotblocks/glost-core`, new `glost` facade package
- Enhanced plugin registry with discovery and validation
- New packages: `@glotblocks/glost-processor`, `@glotblocks/glost-registry`, `@glotblocks/glost-presets`, `@glotblocks/glost-cli`
- Async-only processing (add `await` to all `.process()` calls)

**Quick Start:**
```typescript
// Old (v0.4)
import { processGLOSTWithExtensions } from "@glotblocks/glost-plugins";
const result = processGLOSTWithExtensions(doc, [ext1, ext2]);

// New (v0.5)
import { glost } from "@glotblocks/glost";
const result = await glost().use(ext1).use(ext2).process(doc);
```

---

## Related Documentation

- [Full Migration Guide](/docs/migration/v0.4-to-v0.5) - Complete details and examples
- [Release Notes](/docs/releases/v0.5.0) - What's new in v0.5.0
- [Plugins Guide](/docs/plugins-guide) - Working with the plugin system
- [Provider Philosophy](/docs/provider-philosophy) - Understanding data quality principles

## Need Help?

- **GitHub Issues:** https://github.com/fustilio/glost/issues
- **GitHub Discussions:** https://github.com/fustilio/glost/discussions
