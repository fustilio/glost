...
import { createClauseSegmenterPlugin } from "@glotblocks/glost-clause-segmenter";
import { englishSegmenterProvider } from "@glotblocks/glost-en/segmenter";

// Language-specific segmentation provider
const clausePlugin = createClauseSegmenterPlugin({
  targetLanguage: "en",
...
// 1. Create and freeze processor for reuse
const processor = glost()
  .use(createClauseSegmenterPlugin({
    targetLanguage: "en",
    provider: englishSegmenterProvider
  }))
  .freeze();
...
```typescript
import type { ClauseSegmenterProvider, SegmentationResult } from "@glotblocks/glost-clause-segmenter";
