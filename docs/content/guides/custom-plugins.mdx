...
```typescript
import { processGLOST, getExtension } from '@glotblocks/glost-plugins';

// Get extension by ID
const frequencyExtension = getExtension("frequency");

const result = processGLOST(document, [frequencyExtension, MyExtension]);
```
...
import { glost, createGLOSTWordNode, createSimpleDocument } from '@glotblocks/glost';
...
describe('MyPlugin', () => {
  it('should add custom metadata', async () => {
    ...
    const result = processGLOST(doc, [MyExtension]);

    const processedWord = result.children[0].children[0].children[0];
...
```typescript
import type { Plugin, GLOSTWord, GLOSTExtras } from '@glotblocks/glost';
...
import { createReadingTimeExtension } from './my-extensions';

const readingTime = createReadingTimeExtension({ wordsPerMinute: 180 });

const result = processGLOST(document, [readingTime, frequencyExtension]);

console.log(result.extras?.readingTime);
