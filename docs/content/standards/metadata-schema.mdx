...
Use built-in plugins to enhance metadata display:

```typescript
import { glost } from "glost";
import { frequencyPlugin, difficultyPlugin, posPlugin } from "glost-plugins";

const result = await glost()
  .use(frequencyPlugin)    // Adds display colors/labels
  .use(difficultyPlugin)   // Adds difficulty indicators
  .use(posPlugin)          // Adds POS abbreviations
  .process(document);
...
- [Part of Speech Extension](../packages/plugins.md#partofspeechextension)
