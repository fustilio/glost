 ...
 * ```
 */
export async function executePipeline(document, extensions) {
 ...
 * ```
 */
export function createPipeline(factories) {
    const extensions = [];
 ...
 */
export function analyzeWordEnrichment(document) {
    let total = 0;
    ...
    for (const node of words) {
        total++;
        const hasTranscription = node.transcription && Object.keys(node.transcription).length > 0;
        const hasTranslation = node.extras?.translations &&
        ...
        if (hasFrequency)
            withFrequency++;
        if (hasTranscription && hasTranslation && hasFrequency)
            fullyEnriched++;
    }
    return {
        total,
