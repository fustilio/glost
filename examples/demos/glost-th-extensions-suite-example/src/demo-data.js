 ...
 */
export const DEMO_THAI_VOCABULARY = [
    {
        word: "สวัสดี",
        transcriptions: {
            rtgs: "sawatdi",
 ...
 * ```
 */
export function findDemoThaiWord(word) {
 ...
 * ```
 */
export function isWordInDemoVocabulary(word) {
 ...
 * ```
 */
export function getDemoThaiTranscriptions(word, schemes) {
    ...
    if (schemes && schemes.length > 0) {
        for (const scheme of schemes) {
            const transcription = entry.transcriptions[scheme];
            if (transcription) {
 ...
 * ```
 */
export function getDemoThaiTranslation(word) {
    const entry = findDemoThaiWord(word);
