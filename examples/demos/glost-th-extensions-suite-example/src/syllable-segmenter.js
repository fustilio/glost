 ...
 * @internal
 */
function computeToneFromMark(toneMark) {
    const toneMarkMap = {
        "่": 1, // Mai ek (low tone)
 ...
 * @internal
 */
function parseSyllable(syllableText) {
    const parts = { text: syllableText };
 ...
 * @internal
 */
function createCharacterNodes(parts) {
    const characters = [];
 ...
 * @internal
 */
function createGLOSTSyllable(parts, options) {
 ...
 * @internal
 */
function segmentThaiWord(word, options) {
    // Get word text
 ...
 * @since 0.0.1
 */
export function createThaiSyllableSegmenterExtension(options = {}) {
    const { segmentAllThai = true, includeCharacters = true, computeTones = true, } = options;
    return {
        ...
        transform: (document) => {
            ...
            visit(document, "WordNode", (node) => {
                ...
                const isThai = word.lang?.startsWith("th") ||
                    word.script === "thai" ||
 ...
 * @since 0.0.1
 */
export const ThaiSyllableSegmenterExtension = createThaiSyllableSegmenterExtension();
//# sourceMappingURL=syllable-segmenter.js.map